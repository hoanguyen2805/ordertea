...
const Notification = () => {
    ...
    function displayMessage(notification) {
        if (notification.message === "DONE") {
            ...
            return (
                <>
                    ...
                    <div className="notify-text">
            <span>
              {t("notification.order_store")}
                <b>{notification.store_name}</b>&nbsp; {t("notification.order_payment")}&nbsp;
                <b>
                {notification.total
        ...
        } else if (notification.message === "PAYMENT_REQUESTED") {
            return (
                <>
                    <img src="/done-order.png" alt=""/>
                    <div className="notify-text">
            <span>
              {t("notification.user")} {notification.sender !== undefined ? `${notification?.sender?.fullName}(${notification.sender?.username})` : notification.sender?.username} {t("notification.requestedPayment")} :
               <b>{notification.store_name}</b>&nbsp; - ${notification.product_name}&nbsp; {t("notification.total")}  &nbsp;
                <b>
                {notification.total
                    ? Number(notification.total).toLocaleString("en-US")
                    : 0}
            ...
            <a href={`/restaurant-management/orders/id=${notification.order_id}`} className={"pl-3 flex-1 text-right"} target={"_blank"}>{t("notification.check")}</a>
            </span>
                        <p className="mt-1 time-notify">
                            {displayDateTime(notification.created_at)}
                        </p>
        ...
        }else if (notification.message === "PAYMENT_APPROVED") {
            return (
                <>
                    <img src="/done-order.png" alt=""/>
                    <div className="notify-text">
            <span>
              {t("notification.user")} {notification.sender !== undefined ? `${notification?.sender?.fullName}(${notification.sender?.username})` : notification.sender?.username} {t("notification.approvedPayment")} :
               <b>{notification.store_name}</b>&nbsp; - ${notification.product_name}&nbsp; {t("notification.total")}  &nbsp;
                <b>
                {notification.total
                    ? Number(notification.total).toLocaleString("en-US")
        ...
        }else if (notification.message === "PAYMENT_REJECTED") {
            return (
                <>
                    <img src="/done-order.png" alt=""/>
                    <div className="notify-text">
            <span>
              {t("notification.user")} {notification.sender !== undefined ? `${notification?.sender?.fullName}(${notification.sender?.username})` : notification.sender?.username} {t("notification.rejectPayment")} :
               <b>{notification.store_name}</b>&nbsp; - ${notification.product_name}&nbsp; {t("notification.total")}  &nbsp;
                <b>
                {notification.total
                    ? Number(notification.total).toLocaleString("en-US")
                ...
                . {t("notification.rejectPaymentCheck")}.
            </span>
                        <p className="mt-1 time-notify">
                            {displayDateTime(notification.created_at)}
                        </p>
